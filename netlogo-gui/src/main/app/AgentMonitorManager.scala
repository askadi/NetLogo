...
with org.nlogo.window.Event.LinkParent
{
  ...
  def inspect(agentClass: AgentKind, a0: Agent, radius: Double) {
    ...
    org.nlogo.window.Event.rehash()
    if(agent == null && (agentClass ne AgentKind.Observer))
      window.requestFocus()
    else
