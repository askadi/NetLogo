...
class AgentMonitorEditor(parent: AgentMonitor) extends javax.swing.JPanel
{
  ...
  def agentClass = parent.agentKind
...
with org.nlogo.window.Events.JobRemovedEvent.Handler
{
  ...
  def wrapSource(innerSource: String) {
    ...
    if(parent.agentClass eq AgentKind.Turtle)
      header += " __turtlecode "
    else if(parent.agentClass eq AgentKind.Patch)
      header += " __patchcode "
    else if(parent.agentClass eq AgentKind.Link)
      header += "__linkcode "
